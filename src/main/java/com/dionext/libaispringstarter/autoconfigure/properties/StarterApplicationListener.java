package com.dionext.libaispringstarter.autoconfigure.properties;
...
public class StarterApplicationListener implements ApplicationListener<ApplicationEvent> {
    ...
    public void onApplicationEvent(ApplicationEvent event) {
        if (event instanceof ApplicationEnvironmentPreparedEvent) {
            ...
            Resource resource = new ClassPathResource("application-libaispringstarter.yaml");
            YamlPropertySourceLoader sourceLoader = new YamlPropertySourceLoader();
            try {
                List<PropertySource<?>> yamlTestProperties = sourceLoader.load("libaispringstarterProperties", resource);
